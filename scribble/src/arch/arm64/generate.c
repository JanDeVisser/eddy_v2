...
void generate_function_declaration(ARM64Function *arm_function, IRFunction *function)
{
    ...
    if (function->num_parameters) {
        ...
        for (size_t ix = 0; ix < function->num_parameters; ++ix) {
            ...
            ARM64Variable *arm_param = da_append_ARM64Variable(
    ...
    if (function->kind == FK_SCRIBBLE) {
        ...
        for (size_t op_ix = 0; op_ix < function->operations.size; ++op_ix) {
            ...
            switch (op->operation) {
            case IR_DECL_VAR: {
                ARM64Variable *variable = da_append_ARM64Variable(
                    &scope->variables,
                    (ARM64Variable) {
                        .scope = scope,
                        .kind = VK_LOCAL,
...
void initialize_assembly(Assembly *assembly)
{
    ...
    for (size_t ix = 0; ix < module->functions.size; ++ix) {
        ...
        ARM64Function *arm_function = da_append_ARM64Function(
            &assembly->functions,
            (ARM64Function) {
                .assembly = assembly,
                .function = function,
...
ARM64Context *generate_arm64(BackendConnection *conn, IRProgram *program)
{
    ...
    for (size_t ix = 0; ix < program->modules.size; ++ix) {
        ...
        Assembly *assembly = da_append_Assembly(
            &ctx->assemblies,
            (Assembly) {
                   .ctx = ctx,
                   .module = module,
