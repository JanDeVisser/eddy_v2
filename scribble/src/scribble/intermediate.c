...
__attribute__((unused)) void generate_FUNCTION(BoundNode *node, IRContext *ctx)
{
    ...
    IRFunction *fnc = da_append_IRFunction(
        &module->functions,
        (IRFunction) {
...
__attribute__((unused)) void generate_MODULE(BoundNode *node, IRContext *ctx)
{
    ...
    IRObject  *mod = (IRObject *) da_append_IRModule(
        &program->modules,
        (IRModule) {
             .obj_type = OT_MODULE,
             .program = program,
...
__attribute__((unused)) void generate_PROGRAM(BoundNode *node, IRContext *ctx)
{
    ...
    IRModule *builtin = da_append_IRModule(
        &program->modules,
        (IRModule) {
            .obj_type = OT_MODULE,
            .program = program,
    ...
    IRFunction *statik = da_append_IRFunction(&builtin->functions,
        (IRFunction) {
            .kind = FK_SCRIBBLE,
            .type = (TypeSpec) { .type_id = VOID_ID, .optional = false },
            .name = sv_from("$static") });
    builtin->$static = (int) builtin->functions.size - 1;
