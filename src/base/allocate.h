...
{
    if (!s_alloc) {
#ifdef ALLOCATOR_ARENA_SZ
        s_alloc = allocator_new_with_size(ALLOCATOR_ARENA_SZ);
#else
...
static inline void save_allocator()
{
    allocator_save(get_allocator());
}

static inline void release_allocator()
{
    assert(get_allocator());
    allocator_release(get_allocator());
}

static inline void reset_allocator()
{
    assert(get_allocator());
    allocator_reset(get_allocator());
}
