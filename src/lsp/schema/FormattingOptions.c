...
OptionalJSONValue OptionalFormattingOptions_encode(OptionalFormattingOptions value)
{
    if (value.has_value) {
        return FormattingOptions_encode(value.value);
    } else {
        RETURN_EMPTY(JSONValue);
    }
}

OptionalOptionalFormattingOptions OptionalFormattingOptions_decode(OptionalJSONValue json)
{
    if (!json.has_value) {
        RETURN_EMPTY(OptionalFormattingOptions);
    }
    RETURN_VALUE(OptionalFormattingOptions, FormattingOptions_decode(json));
}
...
OptionalFormattingOptions FormattingOptions_decode(OptionalJSONValue json)
{
    ...
    }
    {
        ...
        value.trimTrailingWhitespace = FORWARD_OPTIONAL(OptionalBool, FormattingOptions, OptionalBool_decode(v0));
    }
    {
        OptionalJSONValue v0 = json_get(&json.value, "insertFinalNewline");
        value.insertFinalNewline = FORWARD_OPTIONAL(OptionalBool, FormattingOptions, OptionalBool_decode(v0));
    }
    {
        OptionalJSONValue v0 = json_get(&json.value, "trimFinalNewlines");
        value.trimFinalNewlines = FORWARD_OPTIONAL(OptionalBool, FormattingOptions, OptionalBool_decode(v0));
    }
...
OptionalJSONValue FormattingOptions_encode(FormattingOptions value)
{
    ...
    json_optional_set(&v1, "tabSize", UInt32_encode(value.tabSize));
    json_optional_set(&v1, "insertSpaces", Bool_encode(value.insertSpaces));
    json_optional_set(&v1, "trimTrailingWhitespace", OptionalBool_encode(value.trimTrailingWhitespace));
    json_optional_set(&v1, "insertFinalNewline", OptionalBool_encode(value.insertFinalNewline));
    json_optional_set(&v1, "trimFinalNewlines", OptionalBool_encode(value.trimFinalNewlines));
