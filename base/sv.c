...
ErrorOrStringView sv_read(int fd, size_t num)
{
    if (!num) {
    ...
    if (read_chars < num) {
        ERROR(StringView, IOError, errno, "Could not read file");
    }
...
unsigned int sv_hash(StringView *sv)
{
    return hash(sv->ptr, sv->length);
}
