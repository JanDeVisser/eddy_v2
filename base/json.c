...
JSONValue json_get_default(JSONValue *value, char const *attr, JSONValue default_)
{
    ...
    if (ret_maybe.has_value) {
        assert(ret_maybe.value.type == default_.type); // FIXME Should be error
...
extern ErrorOrStringView json_must_string(JSONValue *value, char const *attr)
{
    if (value == NULL) {
        ERROR(StringView, ValueError, 0, "Cannot get attribute '%s' from NULL JSON value pointer", attr);
    }
    if (value->type != JSON_TYPE_OBJECT) {
        ERROR(StringView, ValueError, 0, "Cannot get attribute '%s' from JSON object of type '%s'", attr, JSONType_name(value->type));
    }
    JSONValue v = TRY_OPTIONAL(JSONValue, json_get(value, attr), StringView, ValueError, "JSON object has no attribute '%s'", attr);
    if (v.type != JSON_TYPE_STRING) {
        ERROR(StringView, ValueError, 0, "Attribute '%s' of JSON object must be a string but is a '%s'", attr, JSONType_name(v.type));
    }
    RETURN(StringView, v.string);
}
